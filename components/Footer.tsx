...
function StarrySky() {
  ...
  useEffect(() => {
    const generatedStars = Array.from({ length: 40 }).map((_, i) => ({
      id: i,
      top: `${Math.random() * 80}%`, // Keep stars mostly in the upper 80% of the footer
...
function FacebookIcon() {
  return (
    <svg width="40" height="40" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
      <path d="M22 0H2C.897 0 0 .897 0 2v20c0 1.103.897 2 2 2h10.998V14.828h-2.993v-3.473h2.993v-2.556c0-2.96 1.808-4.576 4.444-4.576 1.266 0 2.355.094 2.673.136v3.1h-1.834c-1.439 0-1.717.683-1.717 1.685v2.21h3.42l-.445 3.473h-2.975V24H22c1.103 0 2-.897 2-2V2c0-1.103-.897-2-2-2z" />
    </svg>
  );
}
...
export default function Footer() {
  const { dict, setLocale } = useTranslations();
  ...
  return (
    <footer id="footer" className="pt-24 pb-12 text-center text-sm text-zinc-500 relative overflow-hidden">
      <StarrySky />
      <div className="mx-auto max-w-4xl px-6 flex flex-col items-center gap-8 relative z-10">
        ...
        >
          ...
          <motion.div
            animate={{ x: [-80, 320] }}
            transition={{ duration: 6, repeat: Infinity, ease: "linear", repeatDelay: 0.3 }}
        ...
        {/* Contact Info */}
        <div className="flex flex-col md:flex-row items-center justify-center gap-6 md:gap-12 text-xl md:text-2xl text-zinc-300">
